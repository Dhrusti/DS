...
namespace Helper
{
	public class CommonHelper
	{
		...
		public static int _pageNumber;
		public static int _pageSize;
		public static bool _orderBy;
		...
		public int PageNumber { get { return Convert.ToInt32(_configuration.GetSection("ByDefaultPagination:Page").Value); } set { _pageNumber = Convert.ToInt32(_configuration.GetSection("ByDefaultPagination:Page").Value); } }
		public int PageSize { get { return Convert.ToInt32(_configuration.GetSection("ByDefaultPagination:PageSize").Value); } set { _pageSize = Convert.ToInt32(_configuration.GetSection("ByDefaultPagination:PageSize").Value); } }
		public bool OrderBy { get { return Convert.ToBoolean(_configuration.GetSection("ByDefaultPagination:OrderBy").Value); } set { _orderBy = Convert.ToBoolean(_configuration.GetSection("ByDefaultPagination:OrderBy").Value); } }

		//This method will return list of type T
		public async Task<IEnumerable<T>> GetData<T, P>(string query, P parameter, string connectionId = "EntitiesConnection")
		{
			try
			{
		...
		//This method will not retun any thing
		public async Task SaveData<P>(string query, P parameter, string connectionId = "EntitiesConnection")
		{
			try
			{
				using IDbConnection connection = new SqlConnection(_configuration.GetConnectionString(connectionId));
		...
		public CommonResponse ExecuteCRUDStoreProcedure(string queryText, SqlParameter[] sqlParameters, bool jsonInput = false)
		{
			...
			try
			{
				string connectionString = _configuration.GetConnectionString("ConnectionStrings:EntitiesConnection");
		...
		public string GetRelativeRootPath(bool? IsIncludingFiles = true)
		{
			string directoryPath = IsIncludingFiles != null && IsIncludingFiles == true ? "/Files" : "";
